...
error=1
TREZOR_MONERO_TESTS_URL=${TREZOR_MONERO_TESTS_URL:-https://github.com/ph4r05/monero/releases/download/v0.14.0.2-tests-u14.04/trezor_tests}
TREZOR_MONERO_TESTS_SHA256SUM=${TREZOR_MONERO_TESTS_SHA256SUM:-540c2f9b324b538a32254bde5377dee86b0b8d3ae5b29da8005bcfa274e2ae5c}
TREZOR_MONERO_TESTS_PATH=${TREZOR_MONERO_TESTS_PATH:-./trezor_monero_tests}
...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
  ...
  docker exec $DOCKER_ID apt-get update -qq
  docker exec $DOCKER_ID apt-get install --no-install-recommends --no-upgrade -qq net-tools socat
